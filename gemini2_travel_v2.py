...
def initialize_llm():
    ...
    logger.info(f"Initializing LLM with model: gemini/gemini-2.5-flash")
    if GEMINI_API_KEY:
        logger.info(f"API Key found: {GEMINI_API_KEY[:4]}...{GEMINI_API_KEY[-4:] if len(GEMINI_API_KEY) > 8 else ''}")
    else:
